...
1823+41
 ...
 fitnes_name <- c()
  ...
  knitr::kable(df, col.names = colnames(df), caption = "Table SII.1 Summary of significant total selection differentials measured for 1823 traits (plus 20 PCS) using 3 fitness traits, measured on individuals and populations, across 4 environments.", fixed_thread=T )
...
if(RERUN){
  ...
  pcs_sel_response <- read.table(file="./tables/PC1_2_selectionResponse.tsv",
              header = T, sep="\t")
  knitr::kable(pcs_sel_response, caption = "Table SII.2 Total selection differentials measured for PC axes 1 and 2 with all fitness traits, measured across two environments (Tubingen and Madrid; t/m), two rainfall treatments (high and low; h/l) and two planting densities (individual and population; i/p).", fixed_thread=T )
}
```
...
if(RERUN){
   ...
   gr_sel <- read.table(file="./tables/GRphenotypes_selectionResponse.tsv",
               header=T, sep="\t")
   knitr::kable(gr_sel, caption = "Table SII.5 Significant total selection differentials measured for growth rate related traits associated with 3 fitness traits measured across two environments (Tubingen and Madrid; t/m), two rainfall treatments (high and low; h/l) and two planting densities (individual and population; i/p).", fixed_thread=T )
}
```
...
if(RERUN){
   ...
   dorm_germ_sel <- read.table(file="./tables/DormPerc_phenotypes_selectionResponse.tsv",
               header = T, sep="\t")
   knitr::kable(dorm_germ_sel, caption = "Table SII.6 Significant total selection differentials measured for primary and secondary dormancy related traits associated with 3 fitness traits measured across two environments (Tubingen and Madrid; t/m), two rainfall treatments (high and low; h/l) and two planting densities (individual and population; i/p).", fixed_thread=T )
}
```
...
if(RERUN){
   ...
   dcar_pheno <- read.table(file="./tables/DeltaC13_phenotypes_selectionResponse.tsv",
               header = T, sep="\t")
   knitr::kable(dcar_pheno, caption = "Table SII.7 Total selection differentials measured for Delta_C13 associated with 3 fitness traits measured across two environments (Tubingen and Madrid; t/m), two rainfall treatments (high and low; h/l) and two planting densities (individual and population; i/p).", fixed_thread=T )
}
```
...
```{r, echo=F, eval=T, message=F, warning=F, fig.width=8, fig.height=10, fig.cap="Figure SIII.21 mGWA of flowering time and delta_C13 colored by selection differentials being either positively selected (green) or negatively selected (red). Selection differentials are from top) a GWA with residuals from a model of lifetime fitness as a function of growth rate and bottom) a GWA with residuals from a model of survival as a function of growth rate"}
RERUN=F
if(RERUN){
...
```{r, echo=F, eval=T, message=F, warning=F, fig.width=7, fig.height=4, fig.cap="Figure SIII.22 Top 0.05% SNPs from mGWA of flowering time and deltaC_13, plotted by their effect size estimated in both traits, colored by their selection differential estimated from linear models of left) lifetime fitness as a function of growth rate and right) survival as a function of growth rate."}
RERUN=F
if(RERUN){
  library(dplyr)
  library(data.table)
...
if(RERUN){
  ...
  ditRaw_Uli_dC13_mut_corplot <- ggplot(dittb_raw_Uli_mut) +
    ...
    geom_text(x=-35, y=-29.5, label="r = 0.89, p < 0.007")  ## 0.89 is Pearson r from cor.test below (was mislabeled r2); see figures-supp/FigS26-remake
  ...
  dittb_imp_Uli_mut_corplot <- ggplot(dittb_imp_Uli_mut) +
    ...
    geom_smooth(method="lm", aes(x=Delta_13C, y=mean_dC13_ko), col="black")+
    geom_text(x=-35, y=-29.5, label="r = 0.71, p < 2.5e-5")  ## 0.71 is Pearson r (was mislabeled r2); p recomputed 2026-08-12 = 2.4e-5 with current data (old 1.5e-5 not reproducible)
